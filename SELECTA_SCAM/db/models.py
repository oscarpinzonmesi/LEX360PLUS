...
class Proceso(Base):
    ...
    documentos = relationship(
        "Documento", back_populates="proceso", cascade="all, delete-orphan"
    )
    contabilidad = relationship(
        "Contabilidad", back_populates="proceso", cascade="all, delete-orphan"
    )
    actuaciones = relationship(
        "Actuacion", back_populates="proceso", cascade="all, delete-orphan"
    )
    robot_busquedas = relationship(
        "RobotBusqueda", back_populates="proceso", cascade="all, delete-orphan"
    )
    eventos = relationship(
        "Evento", back_populates="proceso", cascade="all, delete-orphan"
    )
...
class Contabilidad(Base):
    ...
    # 📌 Campo nuevo para manejar papelera
    eliminado = Column(Boolean, default=False)
    ...
    def __repr__(self):
        ...
        try:
            if hasattr(self, "tipo") and self.tipo:
                tipo_nombre = getattr(self.tipo, "nombre", "N/A")
        except Exception:
            tipo_nombre = "N/A"
        return f"<Contabilidad id={self.id}, cliente_id={self.cliente_id}, tipo={tipo_nombre}>"
