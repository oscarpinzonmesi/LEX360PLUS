...
from ...db.models import Contabilidad, TipoContable as TipoContableORM
...
class ContabilidadLogic:
    def __init__(self, contabilidad_db: ContabilidadDB, clientes_logic: ClientesLogic, contabilidad_model: ContabilidadModel, procesos_logic: ProcesosLogic):
        ...
        logger.info("ContabilidadLogic: Inicializada.")
    ...
    def get_contabilidad_data_for_display(self, cliente_id: Optional[int] = None, proceso_id: Optional[int] = None, search_term: Optional[str] = None, tipo_id: Optional[int] = None):
        """
        ÚNICO método para obtener y formatear los datos para la tabla principal.
        """
        self.logger.debug(f"Logic: Obteniendo datos para display con filtros: C:{cliente_id}, P:{proceso_id}, T:{tipo_id}, S:'{search_term}'")
        ...
        self.logger.debug(f"Logic: {len(display_records)} registros formateados para la vista.")
        return display_records

    def update_contabilidad_record(self, record_id, cliente_id, proceso_id, tipo_id, descripcion, valor, fecha):
        """
        Valida y pasa la solicitud de actualización al modelo.
        """
        self.logger.info(f"ContabilidadLogic: Solicitud de actualización para ID {record_id} recibida.")
    ...
    def get_contabilidad_record_raw(self, record_id: int):
        """
        Obtiene los datos de un único registro para el diálogo de edición.
        """
    ...
    # --- El resto de los métodos (get_tipos_contables, get_contabilidad_report_data, etc. se quedan igual) ---
    # (Por brevedad no los pego, pero asegúrate de que estén en tu archivo final)
    def get_tipos_contables(self):
        # (Este método se queda como lo tienes)
    ...
    def get_contabilidad_report_data(self, record_ids: Optional[list] = None, cliente_id: Optional[int] = None, proceso_id: Optional[int] = None, tipo_id: Optional[int] = None, search_term: Optional[str] = None):
        ...
        # ... (resto de la lógica para calcular totales y filtros)
        ...
        return ContabilidadReportData(
            records=records, total_ingresos=total_ingresos, total_gastos=total_gastos,
            saldo_neto=(total_ingresos - total_gastos), filtros="Filtros Aplicados", # Simplificado
