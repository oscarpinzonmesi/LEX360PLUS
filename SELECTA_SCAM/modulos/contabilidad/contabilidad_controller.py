...
class ContabilidadController(QObject):
    # Señales:
    data_loaded = pyqtSignal() # Esta señal parece no usarse activamente, pero se mantiene si es parte de otra lógica.
    data_updated = pyqtSignal(list) # <--- ¡CAMBIO IMPORTANTE! Ahora emite una LISTA de datos
    ...
    documentos_filtrados_loaded = pyqtSignal(list) # Esta señal parece pertenecer a un módulo de documentos.
    record_updated = pyqtSignal(int)
    def __init__(self, model: ContabilidadModel, contabilidad_logic: ContabilidadLogic, clientes_logic: ClientesLogic, procesos_logic: ProcesosLogic, parent=None):
    ...
    def get_filtered_documents(self, cliente_id: int = None, search_term: str = None, tipo_doc_id: int = None):
        try:
            self.operation_failed.emit("Funcionalidad no implementada", "La búsqueda de documentos desde Contabilidad no está activa.")
            pass # Placeholder si documentos_logic no está inicializado
        except Exception as e:
            logger.exception("ContabilidadController (Doc): Error al obtener documentos filtrados.")
            self.operation_failed.emit(f"Error de Carga de Documentos", f"No se pudieron cargar los documentos filtrados: {e}")

        # En: contabilidad_controller.py
    # Reemplaza el método completo con este:

    def get_contabilidad_records_sync(self, cliente_id: int = None, proceso_id: int = None, search_term: str = None, tipo_id: int = None):
        """
        ...
        try:
            ...
            self.logger.info(f"ContabilidadController: Registros de contabilidad emitidos ({len(records)}).")
            ...
            self.logger.exception("ContabilidadController: Error al cargar registros y calcular resumen.")
    ...
    def add_record(self, cliente_id, proceso_id, tipo_id, descripcion, valor, fecha,
        ...
        try:
            ...
            self.contabilidad_logic.add_contabilidad_record(cliente_id, proceso_id, tipo_id, descripcion, valor, fecha)
            self.get_contabilidad_records_sync(cliente_id=current_filter_cliente_id, proceso_id=current_filter_proceso_id)
...
# En: contabilidad_controller.py
    ...
    def update_record(self, record_id, cliente_id, proceso_id, tipo_id, descripcion, valor, fecha):
        ...
        try:
            ...
            self.logger.exception(f"Error al orquestar la actualización del registro: {e}")
            self.operation_failed.emit(f"Error inesperado al actualizar registro: {e}")
            return False


    def delete_record(self, record_id, current_filter_cliente_id: int = None, current_filter_proceso_id: int = None):
        ...
        try:
            ...
            self.error_occurred.emit(str(ve))
            logger.warning(f"ContabilidadController: Error de validación al eliminar registro: {ve}")
            self.operation_failed.emit(f"Fallo al eliminar registro: {ve}")
            return False
        except Exception as e:
            self.error_occurred.emit(f"Error al eliminar registro: {e}")
            logger.exception(f"ContabilidadController: Error inesperado al eliminar registro ID {record_id}.")
            self.operation_failed.emit(f"Error inesperado al eliminar registro: {e}")
            return False

    def get_summary_data(self, cliente_id: int = None, proceso_id: int = None, search_term: str = None, tipo_id: int = None) -> dict:
        """
        ...
        try:
            summary = self.contabilidad_logic.get_summary_data(cliente_id, proceso_id, search_term=search_term, tipo_id=tipo_id) # Nuevo parámetro
            ...
            logger.exception("ContabilidadController: Error al obtener datos de resumen.")
    ...
    def get_all_clientes_sync(self) -> list:
        ...
        try:
            clientes_data = self.clientes_logic.get_all_clientes_for_combobox() # <-- ¡CAMBIO AQUÍ! Usar clientes_logic
            ...
            logger.exception("ContabilidadController: Error al obtener todos los clientes de forma síncrona.")
    ...
    def get_procesos_by_client_for_dialog(self, cliente_id: int):
        ...
        try:
            procesos = self.contabilidad_logic.get_procesos_by_client(cliente_id) # Asume que contabilidad_logic tiene este método
            ...
            logger.exception(f"ContabilidadController: Error al cargar procesos para diálogo para cliente ID {cliente_id}.")
            self.error_occurred.emit(f"Error al cargar procesos: {e}")
            self.procesos_loaded_for_dialog.emit([]) # Emitir lista vacía en caso de error
    ...
    def get_tipos_contables_sync(self) -> list:
        ...
        try:
            ...
            logger.exception("ContabilidadController: Error al obtener tipos contables de forma síncrona.")
    ...
    def get_record_by_id_for_display(self, record_id: int) -> tuple:
        ...
        try:
            ...
            if record:
                ...
                return record
            logger.warning(f"ContabilidadController: No se encontró el registro ID {record_id}.")
            return None
        except Exception as e:
            logger.exception(f"ContabilidadController: Error al obtener y formatear registro ID {record_id}.")
    ...
    def get_all_procesos_sync(self):
        ...
        try:
            ...
            self.logger.error(f"Error al obtener todos los procesos de forma síncrona: {e}")
            return []
        ...
        try:
            ...
            if not records:
                QMessageBox.information(
                    None, "Advertencia", "No hay registros para generar el PDF con los filtros seleccionados."
                )
                return
    ...
    def generar_pdf_de_seleccion(self, record_ids: list):
        ...
        try:
            ...
            self.pdf_generator.generate_summary_pdf(records)
            self.operation_successful.emit("PDF de registros seleccionados generado exitosamente.")
